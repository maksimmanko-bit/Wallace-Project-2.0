...
const TABLE_COLUMNS = {
    receiving: [
        ...
        ['carrier', 'Carrier', 'carrier'], ['status', 'Status', 'status'], ['attachments', 'Attachments']
    ],
    outbound: [
    ...
    trailers: [
        ...
        ['attachments', 'Documents / Issue']
    ],
    warranty: [
        ['reference', 'Job Number', 'job_number'], ['order_type', 'Order Type', 'order_type'], ['claim_type', 'Claim Type', 'claim_type'],
        ['issue', 'Reported Issue'], ['status', 'Status', 'status'], ['customer', 'Customer / IDs'], ['photos', 'Photos']
    ],
    inspections: [
...
function setupEventListeners() {
    ...
    window.addEventListener('scroll', () => {
        const nextCompact = headerCompact ? window.scrollY > 36 : window.scrollY > 120;
        if (nextCompact === headerCompact) return;
        headerCompact = nextCompact;
        document.querySelector('.app-header')?.classList.toggle('compact', headerCompact);
    }, { passive: true });
...
async function generateOrderPdfReport(orderId) {
    ...
    report.fields.forEach(([label, value]) => {
        const cleanValue = value === undefined || value === null || value === '' ? '-' : sanitizePdfText(value);
        const text = /^\d{4}-\d{2}-\d{2}T/.test(cleanValue) ? cleanValue.replace('T', ' ') : cleanValue;
        ...
        doc.text(sanitizePdfText(label).toUpperCase(), margin, y + 4);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(17, 24, 39);
        doc.text(lines, 62, y + 4);
    ...
    if (report.attachments.length) {
        ...
        report.attachments.forEach(name => {
            doc.text(`- ${sanitizePdfText(name)}`, margin, y);
            y += 5;
        });
    }
