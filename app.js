...
function workflowStatusClass(status) {
    if (status === 'Approved' || status === 'Inspected - good to go') return 'received';
    if (status === 'Denied' || status === 'Inspected - damage found') return 'canceled';
    if (status === 'Working on it' || status === 'Request Submitted') return 'in-transit';
    return 'ordered';
}
...
function updateDashboardChrome() {
    ...
    if (tableHead) {
        tableHead.innerHTML = isWarranty ? `
            <tr>
                <th class="sortable" data-sort="job_number">Job Number</th>
                <th class="sortable" data-sort="order_type">Order Type</th>
                <th class="sortable" data-sort="claim_type">Claim Type</th>
    ...
    filteredOrders.forEach(order => {
        ...
        if (isWarrantyTab) {
            ...
            tr.innerHTML = `
                ...
                <td><span class="badge ${workflowStatusClass(order.status)}">${escapeHtml(order.status)}</span></td>
...
function renderAdminShipmentsList() {
    ...
    filtered.forEach(order => {
        ...
        const statusClass = (warranty || inspection) ? workflowStatusClass(order.status) : ((outbound || pickup || trailer) ? 'in-transit' : order.status.toLowerCase().replace(' ', '-'));
