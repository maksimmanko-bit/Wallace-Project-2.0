    ...
    items.forEach(item => {
        ...
        if (isDoors) {
            tr.innerHTML = `
                ...
                <td><span class="stock-size-badge">${escapeHtml(item.size || '-')}</span></td>
            ...
            tr.innerHTML = `
                ...
                <td><span class="stock-type-badge stock-type-${(item.type || '').toLowerCase()}">${escapeHtml(item.type || '-')}</span></td>
                <td class="desc-col">${escapeHtml(item.description || '-')}</td>
                <td style="text-align:center;">
                    <span class="stock-qty-display ${item.qty <= 0 ? 'stock-qty-zero' : ''}">${item.qty ?? 0}</span>
                </td>
