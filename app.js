...
let activeMobilePdfDocument = null;
let currentStockAnalyticsPeriod = 'month';
...
function closeViewerModal() {
    ...
    if (activeMobilePdfDocument?.destroy) {
        try {
            const cleanup = activeMobilePdfDocument.destroy();
            if (cleanup?.catch) cleanup.catch(err => console.warn('PDF cleanup skipped:', err));
        } catch (err) { console.warn('PDF cleanup skipped:', err); }
        activeMobilePdfDocument = null;
    }
    clearGalleryTouchHandlers();
    ...
    try {
        ...
        if (isPdf) {
            ...
            if (isMobileLayoutViewport()) {
                await renderMobilePdfViewer(fileUrl, wrapper);
            } else {
...
function getGalleryZoomTarget() {
    return document.getElementById('galleryImage') || document.getElementById('galleryMobilePdfDocument') || document.getElementById('galleryDocument');
}

function clearGalleryTouchHandlers() {
...
async function renderMobilePdfViewer(fileUrl, wrapper) {
    if (!window.pdfjsLib?.getDocument) {
        wrapper.innerHTML = '<div class="gallery-error"><i class="fa-solid fa-triangle-exclamation"></i><p>PDF viewer is unavailable.</p></div>';
        return;
    }

    if (activeMobilePdfDocument?.destroy) {
        try { await activeMobilePdfDocument.destroy(); } catch (err) { console.warn('Previous PDF cleanup skipped:', err); }
        activeMobilePdfDocument = null;
    }

    window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
    wrapper.innerHTML = `
        <div class="mobile-pdf-scroll" id="galleryMobilePdfScroll">
            <div class="mobile-pdf-document" id="galleryMobilePdfDocument"></div>
        </div>`;

    const scrollShell = document.getElementById('galleryMobilePdfScroll');
    const docEl = document.getElementById('galleryMobilePdfDocument');
    if (!scrollShell || !docEl) return;

    docEl.innerHTML = '<div class="gallery-loader"><i class="fa-solid fa-spinner fa-spin"></i><span>Rendering PDF...</span></div>';
    ...
    try {
        ...
        const pdf = await window.pdfjsLib.getDocument({ data: new Uint8Array(buffer) }).promise;
        activeMobilePdfDocument = pdf;

        docEl.innerHTML = '';
        const availableWidth = Math.max(280, scrollShell.clientWidth - 18);
        ...
        setupGalleryTouchZoom(docEl);
    } catch (err) {
        console.error('Mobile PDF render failed:', err);
        docEl.innerHTML = '<div class="gallery-error"><i class="fa-solid fa-triangle-exclamation"></i><p>PDF could not be rendered. Use Download to open the file.</p></div>';
    }
}
...
function getStockAnalyticsRange(period = currentStockAnalyticsPeriod) {
    const end = new Date();
    ...
    return { start, end, label: labels[period] || labels.month };
}

function isStockNoteInAnalyticsRange(note, range) {
    if (!note?.created_at) return false;
    const date = new Date(note.created_at);
    return !Number.isNaN(date.getTime()) && date >= range.start && date <= range.end;
}

function stockItemMatchesAnalyticsSearch(item, fields, searchVal) {
    if (!searchVal) return true;
    return fields.some(field => item?.[field] && String(item[field]).toLowerCase().includes(searchVal));
}

function collectStockSalesMovement(period, searchVal) {
    const range = getStockAnalyticsRange(period);
    const groups = [
        { type: 'Door', icon: 'fa-door-open', items: stockDoors, fields: ['model', 'size', 'description'], label: item => item.model || 'Door', sub: item => item.size || '' },
        { type: 'Operator', icon: 'fa-gears', items: stockOperators, fields: ['model', 'type', 'description'], label: item => item.model || 'Operator', sub: item => item.type || '' },
        { type: 'Part', icon: 'fa-screwdriver-wrench', items: stockParts, fields: ['model', 'type', 'description'], label: item => item.model || 'Part', sub: item => item.type || '' },
        { type: 'Rail', icon: 'fa-grip-lines', items: stockRails, fields: ['model', 'type', 'size'], label: item => item.model || 'Rail', sub: item => [item.type, item.size ? `${item.size}'` : ''].filter(Boolean).join(' · ') },
        { type: 'Extra', icon: 'fa-cube', items: stockExtras, fields: ['model', 'type', 'size'], label: item => item.model || 'Extra', sub: item => [item.type, item.size].filter(Boolean).join(' · ') },
        { type: 'Non-Standard', icon: 'fa-door-closed', items: stockNonStandardDoors, fields: ['model', 'size', 'color', 'description'], label: item => item.model || 'Non-Standard Door', sub: item => [item.size, item.color].filter(Boolean).join(' · ') }
    ];

    const items = [];
    groups.forEach(group => {
        (group.items || []).forEach(item => {
            if (!stockItemMatchesAnalyticsSearch(item, group.fields, searchVal)) return;
            const notes = Array.isArray(item.notes) ? item.notes : [];
            const sold = notes
                .filter(note => isStockNoteInAnalyticsRange(note, range))
                .reduce((sum, note) => sum + (Number(note.qty) || 0), 0);
            const allTimeSold = notes.reduce((sum, note) => sum + (Number(note.qty) || 0), 0);
            items.push({
                id: item.id,
                type: group.type,
                icon: group.icon,
    ...
    const fast = items
        ...
        .sort((a, b) => b.sold - a.sold || a.label.localeCompare(b.label))
        .slice(0, 8);

    const slow = items
        .filter(item => item.qty > 0 || item.allTimeSold > 0)
        .sort((a, b) => a.sold - b.sold || b.qty - a.qty || a.label.localeCompare(b.label))
        .slice(0, 8);
    ...
    return {
        ...
        activeItems: fast.length
    };
}

function setStockAnalyticsPeriod(period) {
    currentStockAnalyticsPeriod = period;
    renderStockAnalytics('stockAnalyticsView');
    renderStockAnalytics('stockAnalyticsModalBody');
}

function renderStockMovementList(items, maxSold, emptyText) {
    if (!items.length) {
        return `<p class="text-muted stock-movement-empty">${emptyText}</p>`;
    }
    return items.map(item => {
        const percent = maxSold > 0 ? Math.max(4, Math.round((item.sold / maxSold) * 100)) : 4;
        return `
            <div class="stock-movement-row">
                <div class="stock-movement-icon"><i class="fa-solid ${item.icon}"></i></div>
                <div class="stock-movement-main">
                    <div class="stock-movement-title">
                        <strong>${escapeHtml(item.label)}</strong>
                        <span>${escapeHtml(item.type)}</span>
                    </div>
                    <div class="stock-movement-sub">${escapeHtml(item.sub || 'No details')} · In stock: ${item.qty}</div>
                    <div class="stock-movement-track"><div style="width:${percent}%"></div></div>
                </div>
                <div class="stock-movement-count">${item.sold}</div>
            </div>`;
    }).join('');
}
...
function renderStockAnalytics(containerId = 'stockAnalyticsView') {
    ...
    const searchVal = (document.getElementById('stockSearchInput')?.value || '').toLowerCase().trim();
    const movement = collectStockSalesMovement(currentStockAnalyticsPeriod, searchVal);
    const maxMovementSold = Math.max(1, ...movement.fast.map(item => item.sold), ...movement.slow.map(item => item.sold));
    ...
    let html = `
        ...
        <section class="stock-movement-analytics">
            <div class="stock-movement-head">
                <div>
                    <span class="record-drawer-eyebrow">Sales movement</span>
                    <h3><i class="fa-solid fa-chart-line"></i> What sells more and less</h3>
                    <p>${movement.range.label} · ${movement.totalSold} unit${movement.totalSold === 1 ? '' : 's'} sold across ${movement.activeItems} active item${movement.activeItems === 1 ? '' : 's'}.</p>
                </div>
                <div class="stock-analytics-periods" role="group" aria-label="Sales period">
                    ${['day', 'week', 'month', 'year'].map(period => `
                        <button type="button" class="${currentStockAnalyticsPeriod === period ? 'active' : ''}" onclick="setStockAnalyticsPeriod('${period}')">
            ...
            <div class="stock-movement-grid">
                <div class="stock-movement-panel fast">
                    <h4><i class="fa-solid fa-arrow-trend-up"></i> Bought most often</h4>
                    ${renderStockMovementList(movement.fast, maxMovementSold, 'No dated sales in this period yet.')}
                </div>
                <div class="stock-movement-panel slow">
                    <h4><i class="fa-solid fa-arrow-trend-down"></i> Bought less often</h4>
                    ${renderStockMovementList(movement.slow, maxMovementSold, 'No inventory movement data yet.')}
                </div>
            </div>
        </section>
