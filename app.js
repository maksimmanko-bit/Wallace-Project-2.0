...
const TABLE_COLUMNS = {
    receiving: [
        ['reference', 'PO Number', 'po_number'], ['supplier', 'Supplier', 'supplier'], ['description', 'Description / Items'],
        ['employee', 'Ordered By', 'ordered_by'], ['ordered_date', 'Ordered Date', 'ordered_date'], ['eta', 'ETA', 'eta'],
        ['carrier', 'Carrier', 'carrier'], ['status', 'Status', 'status'], ['attachments', 'Attachments'], ['actions', 'Actions']
    ],
    outbound: [
        ['reference', 'Job Number', 'job_number'], ['customer', 'Customer', 'customer'], ['carrier', 'Carrier', 'carrier'],
        ['address', 'Ship To Address'], ['payment', 'Payment Type', 'payment_type'], ['date', 'Shipped Date', 'shipped_date'],
        ['photos', 'Photos'], ['actions', 'Actions']
    ],
    pickup: [
        ['reference', 'Job Number', 'job_number'], ['customer', 'Customer', 'customer_name'], ['employee', 'Handled By', 'handled_by'],
        ['comments', 'Comments'], ['date', 'Pick Up Date', 'pickup_date'], ['photos', 'Photos'], ['actions', 'Actions']
    ],
    trailers: [
        ['reference', 'BOL #', 'bol_number'], ['trip', 'Trip #', 'trip_number'], ['stop', 'Stop #', 'stop_number'],
        ['trailer', 'Trailer / Weight'], ['status', 'Status', 'status'], ['timeline', 'Timeline', 'arrived_date'],
        ['attachments', 'Documents / Issue'], ['actions', 'Actions']
    ],
    warranty: [
        ['reference', 'Job Number', 'job_number'], ['order_type', 'Order Type', 'order_type'], ['claim_type', 'Claim Type', 'claim_type'],
        ['issue', 'Reported Issue'], ['status', 'Status', 'status'], ['customer', 'Customer / IDs'], ['photos', 'Photos'], ['actions', 'Actions']
    ],
    inspections: [
        ['reference', 'Job Number', 'job_number'], ['customer', 'Customer', 'customer_name'], ['status', 'Status', 'status'],
        ['employee', 'Requested / Inspected By'], ['notes', 'Notes'], ['photos', 'Photos'], ['actions', 'Actions']
    ]
};
...
function getSavedColumnOrder(tab = currentDashboardTab) {
    ...
    try {
        ...
        const valid = saved.filter(key => defaults.includes(key) && key !== 'actions');
        defaults.filter(key => key !== 'actions' && !valid.includes(key)).forEach(key => valid.push(key));
...
function openColumnSettings() {
    closeAllModals();
    pendingColumnOrder = getSavedColumnOrder().filter(key => key !== 'actions');
...
function renderColumnSettingsList() {
    ...
    document.getElementById('columnSettingsList').innerHTML = pendingColumnOrder.map((key, index) => `
        ...
        </div>`).join('') + '<div class="column-setting-row locked"><i class="fa-solid fa-lock"></i><strong>Actions</strong><span>Pinned right</span></div>';
}
...
function saveColumnSettings() {
    localStorage.setItem(`max-logistics-columns-${currentDashboardTab}`, JSON.stringify([...pendingColumnOrder, 'actions']));
    closeAllModals();
    renderDashboard();
...
function resetColumnSettings() {
    ...
    pendingColumnOrder = TABLE_COLUMNS[currentDashboardTab].map(column => column[0]).filter(key => key !== 'actions');
    renderColumnSettingsList();
}
    ...
    filteredOrders.forEach(order => {
        ...
        if (isWarrantyTab) {
            ...
            const photoCell = photoCount
                ? `<button class="btn-scan attached" onclick="viewWarrantyPhotos('${order.id}')"><i class="fa-solid fa-camera"></i> Photos (${photoCount})</button>`
            ...
            tr.innerHTML = `
                ...
                <td>
                    ...
                    <select class="quick-status-select" aria-label="Change warranty status" onchange="quickSetWarrantyStatus('${order.id}', this.value)">
                        ${WARRANTY_STATUSES.map(status => `<option value="${status}" ${status === order.status ? 'selected' : ''}>${status}</option>`).join('')}
                    </select>
                </td>
                <td><strong>${escapeHtml(order.customer_name || '-')}</strong><div class="received-info">${escapeHtml(secondaryId || '-')}</div></td>
                <td>${photoCell}</td>
                <td><div class="action-buttons"><button class="btn-table-action" onclick="openWarrantyModal('${order.id}')" title="Edit Warranty Claim"><i class="fa-regular fa-pen-to-square"></i></button><button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report"><i class="fa-regular fa-file-pdf"></i></button></div></td>
            `;
        ...
        if (isInspectionsTab) {
            const photoCount = order.photos?.length || 0;
            const photoCell = photoCount
                ? `<button class="btn-scan attached" onclick="viewInspectionPhotos('${order.id}')"><i class="fa-solid fa-camera"></i> Photos (${photoCount})</button>`
                : '<span class="text-muted">No photos</span>';
            const employeeLabel = order.status === 'Inspection Request'
            ...
            tr.innerHTML = `
                ...
                <td>${photoCell}</td>
                <td><div class="action-buttons"><button class="btn-table-action" onclick="openInspectionModal('${order.id}')" title="Edit Inspection"><i class="fa-regular fa-pen-to-square"></i></button><button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report"><i class="fa-regular fa-file-pdf"></i></button></div></td>
            `;
            appendConfiguredRow(tableBody, tr, order);
            return;
        }
        ...
        if (isTrailersTab) {
            ...
            const statusAction = order.status === 'Expected'
                ? `<button class="btn-table-action receive" onclick="quickSetTrailerStatus('${order.id}', 'Arrived')" title="Mark Arrived"><span class="quick-action-label"><i class="fa-solid fa-warehouse"></i> Arrived</span></button>`
                : order.status === 'Arrived'
                ? `<button class="btn-table-action receive" onclick="quickSetTrailerStatus('${order.id}', 'Dispatched')" title="Mark Dispatched"><span class="quick-action-label"><i class="fa-solid fa-truck-fast"></i> Dispatched</span></button>`
                : '';
            ...
            tr.innerHTML = `
                ...
                <td><div class="action-buttons">${statusAction}${emailButton}<button class="btn-table-action" onclick="openTrailerModal('${order.id}')" title="Edit Trailer"><i class="fa-regular fa-pen-to-square"></i></button><button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report"><i class="fa-regular fa-file-pdf"></i></button></div></td>
            `;
            appendConfiguredRow(tableBody, tr, order);
            return;
        }
...
function setupEventListeners() {
    ...
    window.addEventListener('scroll', () => document.querySelector('.app-header')?.classList.toggle('compact', window.scrollY > 72), { passive: true });
...
async function generateOrderPdfReport(orderId) {
    ...
    const drawPdfLogo = () => {
        ...
        const logoY = 6.3;
        doc.addImage(pdfLogoDataUrl, 'PNG', logoX, logoY, logoWidth, logoHeight, 'maxm-logo', 'FAST');
    };
...
function viewOrderDetails(orderId) {
    const order = orders.find(o => o.id == orderId);
    if (!order) return;
    const timelineHtml = buildRecordTimeline(order);
